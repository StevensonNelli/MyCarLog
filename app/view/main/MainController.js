...
Ext.define('MyCarLog.view.main.MainController', {
    ...
    control: {
        ...
        'carslist [reference=carlist]': {
            'itemtap': 'onCarslistItemTap',
            //'refresh': 'onCarslistRefresh'
            //'added': 'onCarslistRefresh'
        },
    ...
    onActiveItemChange: function(card, newActiveItem, oldActiveItem, eOpts) {
        ...
        if (newActiveItem.xtype != 'welcome' && newActiveItem.xtype != 'carslist') {
            this.lookupReference('fabBack').setHidden(false);
        }

        if (newActiveItem.xtype == 'carslist' || newActiveItem.xtype === 'welcome') {
    ...
    //     var query = '',
    //         renderBtn = function(querySelector) {
    //             var renderSelector = Ext.query(querySelector);

    //             if(!renderSelector || renderSelector.length === 0) {
    //                 renderBtn(querySelector);
    //             }
    //             debugger;
    //             for (var i in renderSelector) {
    //                 if (!renderSelector[i].innerHTML.length) {
    ...
    //     //renderBtn('list div.actions');
    //     //renderBtn('dataview div.actions');
    //     query = (MyCarLog.deviceInfo.tablet) ? '.x-dataview div.actions' :
    //         '.x-list div.actions';
    ...
    fabAction: function() {
        ...
        if (activeXtype === "carslist") {
            ...
            form = Ext.create('MyCarLog.view.forms.AddCar', {});
    ...
    saveCarForm: function(form) {
        ...
        // initializing the car info values
        carInfo['currentMeterReading'] = carInfo['initialMeterReading'];
        // setting a default image if no image is uploaded
        if(carInfo.image === null) {
            carInfo.image = MCLUtil.DEFAULT_CAR_IMG;
        }

        MCLFormUtil.saveCar(
            carInfo, // form values
            Object.keys(MyCarLog.model.CarMeta.getFieldsMap()), // meta data fields
            Object.keys(MyCarLog.model.Car.getFieldsMap()), // rest of the data fields
            viewModel.get('carsmeta') // meta store
        );

        form.destroy();
    },
    saveMaintenanceForm: function(form) {
        console.log('save mnt form');
        form.destroy();
    },
